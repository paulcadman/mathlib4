...
/--
Evaluates an expression `e` into a normalized representation as a polynomial, returning
`none` when there is nothing to do: when the type of `e` is not a commutative semiring, or
when `e` is an atom for the `ring` tactic.

This is a variant of `Mathlib.Tactic.Ring.eval`, the main driver of the `ring` tactic,
operating on `Expr` (input) and `Simp.Result` (output) rather than typed `Qq` versions of
these.
-/
