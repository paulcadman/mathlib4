...
structure EvalResult (M : Q(Type v)) where
  /-- The normal form `c₁ • x₁ + (c₂ • x₂ + ⋯ + 0)`, with ring-normalized scalars. -/
  expr : Q($M)
  /-- A proof (as a raw `Expr`) that the input expression equals `expr`. -/
  proof : Expr
  /-- Whether the input was a single unit-coefficient atom, in which case `expr` is merely a
...
def eval {M : Q(Type v)} (iM : Q(AddCommMonoid $M)) (e : Q($M)) : AtomM (EvalResult M) := do
  let ⟨_, _, iR, iRM, l, pf⟩ ← parse iM e
  let isAtom ← match l with
    | [((_, x), _)] => withTransparency (← read).red <| isDefEq x e
    | _ => pure false
  let ⟨e, pf_rebuild⟩ ← qNF.rebuild iM iR iRM l
  return { expr := e, proof := ← mkEqTrans pf pf_rebuild, isAtom }
