...
def evalExpr (e : Expr) : AtomM Simp.Result := do
  ...
  let r ← Mathlib.Tactic.Module.eval iM e
...
def moduleNFCore (s : IO.Ref AtomM.State) (e : Expr) : MetaM Simp.Result :=
  AtomM.recurse s {} (wellBehavedDischarge := true) evalExpr cleanup e
...
elab (name := moduleNF) "module_nf" loc:(location)? : tactic => do
  ...
  let s ← IO.mkRef {}
  transformAtNondepPropLocation (liftM ∘ moduleNFCore s) "module_nf" loc .error false
