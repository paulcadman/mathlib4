...
abbrev Eq1 (A : Matrix (Fin n) (Fin n) R) (p : ℕ) : Prop :=
  (Spec.diagTerm A + Spec.tailSum A)^[p] A =
...
theorem paper_eq2 (i : Fin n) (hEq1 : Eq1 A p) :
    -Spec.diagSum ((Spec.diagTerm A + Spec.tailSum A)^[p] A) i =
...
theorem iter_succ_entry (i j : Fin n) :
    ((Spec.diagTerm A + Spec.tailSum A)^[p + 1] A) i j =
      -Spec.diagSum ((Spec.diagTerm A + Spec.tailSum A)^[p] A) i * A i j
      + ∑ k ∈ Finset.Ioi i,
          ((Spec.diagTerm A + Spec.tailSum A)^[p] A) i k * A k j := by
  rw [Function.iterate_succ_apply', Pi.add_apply, Matrix.add_apply]
  rw [Spec.diagTerm_apply, Spec.tailSum_apply, sum_Ioi_eq_sum_ite]
...
theorem iter_get_eq_matrix_iterate (A : Array R) (hA : A.size = n * n) (t : ℕ)
    (i j : Fin n) :
    BirdDet.iter n A t (BirdDet.get n A) i.val j.val =
      (Spec.diagTerm (Matrix.ofArray (m := n) (n := n) A hA)
        + Spec.tailSum (Matrix.ofArray A hA))^[t]
  ...
  | succ t ih =>
    rw [BirdDet.iter_succ, stepEntry_eq, Function.iterate_succ_apply', Pi.add_apply,
      Matrix.add_apply]
    rw [Spec.diagTerm_apply, Spec.tailSum_apply, Spec.diagSum_eq, sumFrom_fin_tail,
      sumFrom_fin_tail]
...
theorem birdDet_eq_birdDetSpec (A : Array R) (hA : A.size = n * n) :
  ...
  | succ k =>
    ...
    exact iter_get_eq_matrix_iterate A hA k 0 0
