...
```
-/
@[expose] protected def iter (n : ℕ) (A : Array R) (t : ℕ) (F : ℕ → ℕ → R) :
    ℕ → ℕ → R :=
  match t with
  | 0 => F
...
theorem iter_succ (n : ℕ) (A : Array R) (t : ℕ) (F : ℕ → ℕ → R) (i j : ℕ) :
    BirdDet.iter n A (t + 1) F i j =
      stepEntry n A (BirdDet.iter n A t F) i j := rfl
...
theorem birdDet_succ (k : ℕ) (A : Array R) :
    birdDet (k + 1) A =
      (-1 : R) ^ k * BirdDet.iter (k + 1) A k (BirdDet.get (k + 1) A) 0 0 := rfl
