...
theorem charpoly_submatrix_equiv (e : n ≃ m)
...
theorem charpoly_eq_of_mul_eq_mul [IsDomain R] {A B P : Matrix n n R}
    (hP : P.det ≠ 0) (h : A * P = P * B) : A.charpoly = B.charpoly := by
  have hdet : (P.map C).det = C P.det := (RingHom.map_det C P).symm
  refine mul_left_cancel₀ (a := (P.map C).det) ?_ ?_
  · rw [hdet, C_ne_zero]
    exact hP
  · rw [hdet]
    exact C_det_mul_charpoly_of_mul_eq_mul h
